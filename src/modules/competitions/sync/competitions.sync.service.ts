...
export class CompetitionsSyncService {
  ...
  async syncNewOnly(sportAliases: string[] = ['cs2']) {
    ...
    for (const sport of validSports) {
      ...
      this.logger.log(`▶️ Starting NEW-ONLY sync for sport=${sport}`);
      ...
      for (const c of competitions) {
        ...
        if (processed % LOG_EVERY === 0) {
          ...
          this.logger.log(
            `⏳ ${sport}: ${processed} new processed, ${skipped} skipped (${elapsed}s)`
          );
        }
      }
      ...
      this.logger.log(
        `✅ Finished ${sport}: ${processed} NEW synced, ${skipped} skipped in ${sportElapsed}s`
      );
    }

    this.logger.log(
      `🏁 Sync complete. Total NEW: ${totalSynced}, Skipped: ${totalSkipped}`
    );
